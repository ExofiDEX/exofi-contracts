...
contract Multicall2 is IMulticall2
{
    function aggregate(
        Call[] memory calls
    ) override public returns (uint256 blockNumber, bytes[] memory returnData) {
    ...
    function getBlockHash(
        uint256 blockNumber
    ) override public view returns (bytes32 blockHash) {
        blockHash = blockhash(blockNumber);
    }

    function getBlockNumber() override public view returns (uint256 blockNumber) {
        blockNumber = block.number;
    }

    function getCurrentBlockCoinbase() override public view returns (address coinbase) {
    ...
    function getCurrentBlockGasLimit() override public view returns (uint256 gaslimit) {
    ...
    function getLastBlockHash() override public view returns (bytes32 blockHash) {
    ...
    function tryAggregate(
        bool requireSuccess,
        Call[] memory calls
    ) override public returns (Result[] memory returnData) {
